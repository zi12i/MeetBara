...
export default function ProjectManagement() {
  ...
  // 💡 [해결] 윈도우 컨펌 없이 커스텀 모달에서 바로 실행되는 삭제 로직
  ...
  const handleSetPM = (memberId: string) => {
    setProjects(prev => prev.map(p => {
      ...
      return { ...p, members: updatedMembers, owner: updatedMembers.find(m => m.id === memberId)?.name || "" };
    }));
  };
          ...
          <aside className="w-[520px] h-full bg-white rounded-[24px] shadow-sm border border-gray-200 flex flex-col shrink-0 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar bg-gray-50/20 pb-20">
              {projects.map(p => {
                ...
                return (
                  <div key={p.id} onClick={() => setSelectedProjectId(p.id)} className={`group relative p-5 rounded-[22px] cursor-pointer border-2 transition-all flex flex-col gap-3 ${selectedProjectId === p.id ? "bg-white border-[#91D148] shadow-lg scale-[1.01]" : "bg-white border-transparent shadow-sm"}`} style={{ borderLeft: `8px solid ${p.color}` }}>
                    <div className="flex justify-between items-start">
                      <h3 className="font-black text-[16px] text-gray-900 leading-tight pr-10">{p.name} <span className="text-gray-400 font-bold ml-1 text-[12px]">({p.startDate || "미정"} ~ {p.endDate || "미정"})</span>{p.owner && <span className="font-black ml-2 text-[12px]" style={{ color: p.color }}>- {p.owner}</span>}</h3>
                      <div className="absolute right-4 top-5 flex gap-1 opacity-0 group-hover:opacity-100">
                        ...
                        {/* 💡 win.confirm 대신 setDeleteTarget 호출 */}
              ...
              <div className="flex flex-col h-full overflow-hidden">
                <div className="p-10 pb-6 shrink-0 bg-white">
                  <div className="flex items-center justify-between mb-4">
                    <h1 className="text-[26px] font-black text-gray-900 tracking-tight">{selectedProject.name} <span className="text-[18px] text-gray-300 font-bold ml-2">/ {selectedProject.owner} PM</span></h1>
                  ...
                  <div className="flex items-center gap-2 text-[14px] font-bold text-gray-400 bg-gray-50 w-fit px-4 py-1.5 rounded-full border border-gray-100">
          ...
          <div className="bg-white w-full max-w-md rounded-[32px] p-10 shadow-2xl relative animate-zoom-in">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-[13px] font-bold text-gray-700 mb-1">시작일</label><input type="date" value={formData.start} onChange={(e) => setFormData({...formData, start: e.target.value})} className="w-full h-14 bg-gray-50 rounded-xl px-4 font-bold outline-none" /></div>
                <div><label className="block text-[13px] font-bold text-gray-700 mb-1">종료일</label><input type="date" value={formData.end} onChange={(e) => setFormData({...formData, end: e.target.value})} className="w-full h-14 bg-gray-50 rounded-xl px-4 font-bold outline-none" /></div>
              </div>
