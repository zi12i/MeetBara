...
const QuickMeeting: React.FC = () => {
  ...
  const [agendas, setAgendas] = useState<Agenda[]>([
    { id: 1, text: "안건을 입력하세요", isCompleted: false },
  ]);
  const [activeAgendaId, setActiveAgendaId] = useState<number>(1);
  ...
  const [baraChatList, setBaraChatList] = useState<ChatMessage[]>([
    { id: 1, sender: "bara", text: "안녕하세요! 회의바라입니다. 빠른 회의도 꼼꼼하게 기록해 드릴게요. 🐹", time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }
  ]);
  ...
  const [currentBaraId, setCurrentBaraId] = useState("meeting_quick");
              ...
              content: `당신은 회의 스크립트를 분석하는 전문 AI 어시스턴트입니다. 제공된 대화 내용을 바탕으로 실시간 요약과 핵심 키워드를 JSON 형식으로만 반환하세요.
{
  ...
  return (
    <div className="h-screen bg-[#F8F9FA] flex flex-col font-sans overflow-hidden">
      <PageMeta title={`빠른 회의 - ${id}`} description="빠른 회의 진행 화면" />
