            ...
            <div className="flex-1 overflow-y-auto space-y-6 px-2 no-scrollbar pb-10">
              {activeTab === "decisions" ? (
                <section className="space-y-4 animate-fade-in h-full">
                  {agendas.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center text-gray-400 space-y-3 pb-10">
                      <div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center text-2xl mb-2 border border-gray-100">📝</div>
                      <p className="font-bold text-[16px] text-gray-500">아직 등록된 안건이 없습니다.</p>
                      <p className="text-[14px]">회의 내용을 바탕으로 안건을 자동 추출합니다.</p>
                    </div>
                  ) : (
                    agendas.map((agenda) => (
                ...
                <div className="animate-fade-in space-y-8 h-full">
                  {liveSummaries.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center text-gray-400">
                      <p className="font-bold">대화가 일정 시간 진행되면 실시간 요약을 생성합니다.</p>
                    </div>
                  ) : (
            ...
            <div className="flex justify-center w-full px-4 mb-8 shrink-0">
              <div className="flex items-center bg-[#F1F3F5] rounded-full px-10 py-2.5 shadow-sm border border-gray-200 gap-6">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <button onClick={() => { setIsStopModalOpen(true); }} className="flex items-center gap-2 bg-white border border-gray-200 px-4 py-1.5 rounded-full hover:bg-gray-50 shadow-sm">
              ...
              {activeSideTab === "keywords" ? (
                <div className="grid grid-cols-2 gap-4 animate-fade-in pb-10 overflow-y-auto no-scrollbar">
                  {meetingKeywords.length === 0 ? (
                    <div className="col-span-2 flex items-center justify-center h-full text-gray-400 font-bold">키워드 추출 중...</div>
                  ) : (
                    meetingKeywords.map((kw, idx) => (
                      <div key={idx} onClick={() => { setActiveSideTab("keyword-search"); setSearchTerm(kw); }} className="bg-gray-200/80 border border-gray-300 py-6 px-4 rounded-[20px] flex items-center justify-center shadow-sm hover:bg-[#91D148] hover:text-white cursor-pointer group animate-fade-in-up">
                ...
                <div className="space-y-6 animate-fade-in overflow-y-auto no-scrollbar pt-2 h-full">
                  {filteredLogs.length === 0 ? (
                    <div className="flex items-center justify-center h-full text-gray-400 font-bold">검색 결과가 없습니다.</div>
                  ) : (
