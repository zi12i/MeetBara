...
const LiveMeeting: React.FC = () => {
  ...
  const [isCarryOverModalOpen, setIsCarryOverModalOpen] = useState(false); // ★ 안건 이월 모달 상태
  ...
  useEffect(() => {
    ...
    // ★ 10분 전 알림 토스트 (프로토타입: 38초)
      ...
      if (agendas.some(a => !a.isCompleted)) {
        ...
        setToastSubMessage(`진행률 : ${progress}%`); // ★ 진행률 텍스트 세팅
        setIsToastVisible(true);
      }
    }
    // ★ 회의 종료 이월 팝업 (프로토타입: 45초)
    ...
    if (targetBaraId !== currentBaraId) setCurrentBaraId(targetBaraId);
    if (isDeviation !== isDeviationDetected) setIsDeviationDetected(isDeviation);
  ...
  useEffect(() => {
    ...
    // 녹음 중지 모달이 열려있지 않을 때만 바라 상태 업데이트 (종료 모달 덮어쓰기 방지)
    if (!isStopModalOpen) {
      emitBara(currentBaraId, calculatedProgress);
    }
  }, [agendas, currentBaraId, isStopModalOpen]);
  ...
  return (
    <>
      ...
      <Toast 
        ...
        subMessage={toastSubMessage} // ★ 서브 메시지 전달 추가
              ...
              <div className="flex items-center gap-6">
                ...
                <button 
                  onClick={() => { 
                    ...
                    setCurrentBaraId("meeting_done"); // ★ 상태 강제 업데이트
                    emitBara("meeting_done"); 
                  }} 
                  className="flex items-center gap-2 bg-white border border-gray-200 px-4 py-1.5 rounded-full hover:bg-gray-50 shadow-sm"
                >
                  <div className="w-2.5 h-2.5 bg-[#FF6B6B] rounded-sm"></div>
                  <span className="text-[13px] font-bold text-[#495057]">녹음 종료</span>
                </button>
              </div>
            </div>
          </div>
            ...
            {activeSideTab === "keyword-search" && (
              <div className="space-y-4 animate-fade-in">
                <div className="relative group">
                  ...
                  {searchTerm && (
                    <button onClick={() => setSearchTerm("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M15 9l-6 6M9 9l6 6"/></svg></button>
                  )}
                </div>
            ...
            <div className="flex gap-3">
              <button 
                onClick={() => { 
                  ...
                  setCurrentBaraId(originBaraId); // 까먹은 표정 다시 주입!

                  // 2. 진행률 다시 계산해서 전역 이벤트 발송
                  const completedCount = agendas.filter(a => a.isCompleted).length;
                  const progress = agendas.length > 0 ? Math.floor((completedCount / agendas.length) * 100) : 0;
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-[24px] p-10 w-[420px] shadow-2xl text-center animate-fade-in">
            ...
            <p className="text-[14px] font-medium text-gray-600 mb-8 leading-relaxed">
              지정된 회의 시간이 종료되었습니다.<br />해결되지 않은 안건을 이월하시겠습니까?
            </p>
            <div className="flex gap-3">
              ...
              {/* ★ 이월하기 클릭 시 -> 알림 후 종료 모달로 자동 연결 */}
              <button onClick={() => { alert("미결정 안건이 다음 회의로 이월되었습니다! 🐹"); setIsCarryOverModalOpen(false); setIsStopModalOpen(true); emitBara("meeting_done"); }} className="flex-1 py-3.5 bg-[#91D148] text-white font-bold rounded-xl hover:bg-[#82bd41] shadow-[0_4px_12px_rgba(145,209,72,0.3)]">이월하기</button>
            </div>
          </div>
        </div>
      )}
