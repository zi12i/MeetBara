...
const generateTodayMeetings = () => {
  ...
  return [
    ...
    },
    {
      ...
      projectColor: "#3b82f6", // 파란색 라벨 (신규 프로젝트)
      briefing: {
    ...
    },
    {
      ...
      room: "대회의실",
      attendees: "박인사, 최채용",
      projectColor: "#f59e0b", // 주황색 라벨 (채용 프로젝트)
      briefing: {
...
export default function MeetingStart() {
  ...
  return (
    <>
      ...
      <div className="w-full h-[calc(100vh-90px)] lg:h-[calc(100vh-100px)] flex flex-col lg:flex-row gap-6 p-6 bg-[#FAFAFA] overflow-hidden">
        
        {/* === 좌측: 오늘 예정된 회의 리스트 === */}
        <div className="w-full lg:w-[380px] bg-white rounded-[24px] shadow-sm border border-gray-100 flex flex-col shrink-0 overflow-hidden">
          <div className="p-6 pb-4 border-b border-gray-50 bg-white z-10 shrink-0">
            <h2 className="text-[18px] font-black text-gray-900 flex items-center gap-2.5">
              <span className="text-gray-800"><CalendarIcon /></span>
              오늘 예정된 회의
              <span className="bg-[#91D148]/10 text-[#91D148] text-[12px] px-2.5 py-1 rounded-full ml-1">{meetings.length}건</span>
            </h2>
          </div>
            ...
            {meetings.length > 0 ? (
              meetings.map((meeting) => (
                <div 
                  ...
                  // 👉 핵심 수정: 촌스러운 내부 div 삭제, 카드 자체의 border-l(왼쪽 테두리) 두께를 6px로 키움
                  className={`p-5 rounded-2xl cursor-pointer transition-all border border-gray-100 border-l-[6px] overflow-hidden ${
                    selectedMeeting?.id === meeting.id 
                      ? "bg-[#F4F9ED] shadow-md" 
                      : "bg-white shadow-sm hover:shadow-md hover:bg-gray-50/50"
                  }`}
                  // 👉 왼쪽 테두리 색상만 프로젝트 컬러로 덮어씌움
                  style={{ borderLeftColor: meeting.projectColor }}
                >
                  {/* 시간 표시는 프로젝트 라벨 색상과 맞춰 통일감 부여 */}
                  <p className="text-[12px] font-black mb-2" style={{ color: meeting.projectColor }}>{meeting.timeStr}</p>
                  <h3 className="font-bold text-[15px] text-gray-900 mb-4 leading-snug line-clamp-2">{meeting.title}</h3>
                  <div className="flex flex-col gap-2 text-[12px] text-gray-500 font-medium">
                    <p className="flex items-center gap-2"><span className="text-gray-400"><MapPinIcon /></span> {meeting.room}</p>
                    <p className="flex items-center gap-2"><span className="text-gray-400"><UsersIcon /></span> {meeting.attendees}</p>
                  </div>
                </div>
              ))
            ) : (
        ...
        <div className="flex-1 bg-white rounded-[24px] shadow-sm border border-gray-100 flex flex-col overflow-hidden relative">
          {selectedMeeting ? (
            <>
              {/* 브리핑 헤더 */}
              <div className="p-8 pb-6 border-b border-gray-100 shrink-0 relative">
                <div className="absolute top-0 right-0 p-8 opacity-5 pointer-events-none">
                  <svg width="120" height="120" viewBox="0 0 24 24" fill="currentColor"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                </div>
                <div className="flex items-center gap-2 mb-4">
                  <span className="bg-[#91D148] text-white text-[12px] font-black px-3 py-1.5 rounded-lg shadow-sm">AI 사전 브리핑</span>
                  <span className="w-2 h-2 rounded-full ml-1" style={{ backgroundColor: selectedMeeting.projectColor }}></span>
                  <span className="text-gray-400 text-[13px] font-bold px-1">{selectedMeeting.timeStr}</span>
                </div>
                <h2 className="text-[24px] lg:text-[28px] font-black text-gray-900 mb-6 leading-tight tracking-tight">
                  {selectedMeeting.title}
                </h2>
                <div className="flex gap-6 text-[14px] font-bold text-gray-600 bg-gray-50 py-3 px-5 rounded-xl inline-flex border border-gray-100">
                  <p className="flex items-center gap-1.5"><span className="text-gray-400"><MapPinIcon /></span> {selectedMeeting.room}</p>
                  <div className="w-[1px] h-4 bg-gray-300 my-auto"></div>
                  <p className="flex items-center gap-1.5"><span className="text-gray-400"><UsersIcon /></span> 참석자: {selectedMeeting.attendees}</p>
                </div>
              </div>

              {/* 브리핑 컨텐츠 스크롤 영역 */}
              <div className="flex-1 overflow-y-auto p-8 space-y-6 no-scrollbar">
                <div className="bg-[#F4F9ED]/60 border border-[#91D148]/20 rounded-2xl p-6 shadow-sm">
                  <h4 className="flex items-center gap-2 font-black text-[#628A31] text-[16px] mb-4">
                    <MemoIcon /> 지난 회의 요약
                  </h4>
                  <div className="text-[14px] text-gray-700 font-medium leading-relaxed whitespace-pre-wrap pl-1">
                    {selectedMeeting.briefing?.lastSummary}
                  </div>
                </div>

                <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-6 shadow-sm">
                  <h4 className="flex items-center gap-2 font-black text-blue-600 text-[16px] mb-4">
                    <TargetIcon /> 오늘의 목표
                  </h4>
                  <div className="text-[14px] text-gray-700 font-medium leading-relaxed whitespace-pre-wrap pl-1">
                    {selectedMeeting.briefing?.todaysGoal}
                  </div>
                </div>

                <div className="bg-red-50/50 border border-red-100 rounded-2xl p-6 shadow-sm mb-10">
                  <h4 className="flex items-center gap-2 font-black text-red-500 text-[16px] mb-4">
                    <WarningIcon /> 예상 이슈 및 리스크
                  </h4>
                  <div className="text-[14px] text-gray-700 font-medium leading-relaxed whitespace-pre-wrap pl-1">
                    {selectedMeeting.briefing?.expectedIssues}
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 bg-white border-t border-gray-100 flex justify-end shrink-0">
                <button 
                  ...
                  className="bg-[#91D148] text-white px-10 py-4 rounded-xl font-black text-[16px] shadow-[0_4px_12px_rgba(145,209,72,0.3)] hover:bg-[#82bd41] hover:-translate-y-0.5 transition-all flex items-center gap-2 group"
                >
                  회의 시작하기
