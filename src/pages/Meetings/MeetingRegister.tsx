...
// 💡 추가된 첨부파일 아이콘
const PaperclipIcon = () => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"></path></svg>;
...
  setAttachments([]); // 💡 폼 초기화 시 첨부파일 초기화
};
...
  const [isTimeErrorModalOpen, setIsTimeErrorModalOpen] = useState(false);
...
  const [isDateDropdownUp, setIsDateDropdownUp] = useState(false);
  const datePickerRef = useRef<HTMLDivElement>(null);
  const startTimeRef = useRef<HTMLDivElement>(null);
  const endTimeRef = useRef<HTMLDivElement>(null);
  const timeStateRef = useRef({ start: startTime, end: endTime });

  useEffect(() => {
    timeStateRef.current = { start: startTime, end: endTime };
  }, [startTime, endTime]);

  const checkDateSpace = () => {
    if (datePickerRef.current) {
      const rect = datePickerRef.current.getBoundingClientRect();
      const spaceBelow = window.innerHeight - rect.bottom;
      setIsDateDropdownUp(spaceBelow < 320);
    }
  };
...
      setSelectedTemplate(meeting.templateName || "");
      ...
      // 💡 기존 회의 선택 시 첨부자료 연동 (기존 데이터에 있다면)
...
    project: selectedProject || "프로젝트 미지정",
...
  const handleTimeWheelNative = (e: WheelEvent, isStart: boolean) => {
    e.preventDefault(); // 화면 스크롤 방지
    const { start, end } = timeStateRef.current;
    const currentVal = isStart ? start : end;
    
    // 값이 비어있을 경우 "12:00"을 기준으로 휠 조절 시작
    const timeStr = currentVal || "12:00"; 
    
    const [h, m] = timeStr.split(':').map(Number);
    const dateObj = new Date();
    dateObj.setHours(h, m, 0, 0);
    const direction = e.deltaY < 0 ? 1 : -1;
    dateObj.setMinutes(dateObj.getMinutes() + (direction * 5));
    
    const newH = String(dateObj.getHours()).padStart(2, '0');
    const newM = String(dateObj.getMinutes()).padStart(2, '0');
    const newTime = `${newH}:${newM}`;

    // 시작 시간이 종료 시간보다 커지거나, 종료 시간이 시작 시간보다 작아지는 것 방지
    if (isStart) {
...
  useEffect(() => {
    const sNode = startTimeRef.current;
    const eNode = endTimeRef.current;
    const onStartWheel = (e: WheelEvent) => handleTimeWheelNative(e, true);
    const onEndWheel = (e: WheelEvent) => handleTimeWheelNative(e, false);

    sNode?.addEventListener("wheel", onStartWheel, { passive: false });
    eNode?.addEventListener("wheel", onEndWheel, { passive: false });
    return () => {
      sNode?.removeEventListener("wheel", onStartWheel);
      eNode?.removeEventListener("wheel", onEndWheel);
    };
  }, []);

  // 수동 클릭 시에도 제약 조건 적용
  const handleStartTimeChange = (val: string) => {
    if (endTime && val >= endTime) { setIsTimeErrorModalOpen(true); return; }
    setStartTime(val);
  };

  const handleEndTimeChange = (val: string) => {
    if (startTime && val <= startTime) { setIsTimeErrorModalOpen(true); return; }
    setEndTime(val);
  };
...
    selectedPendingAgendas: selectedPendingAgendas,
    attachments: attachments, // 💡 히스토리 및 회의 일정 페이지 연동을 위한 첨부파일 저장
  };
...
                    </label>
                    
                    <div onClick={checkDateSpace} className={`relative [&>div>div:last-child]:!mb-2 [&>div]:!w-full [&>div>div:first-child]:h-[54px] [&>div>div:first-child]:text-[15px] ${isDateDropdownUp ? '[&>div>div:last-child]:!bottom-full [&>div>div:last-child]:!top-auto' : '[&>div>div:last-child]:!top-full [&>div>div:last-child]:!bottom-auto'}`}>
                      <DatePicker 
                        value={date} 
                        onChange={setDate}
                        placeholder="날짜 선택" 
...
                      className="w-full bg-white border border-gray-200 rounded-xl px-5 py-4 text-left text-[15px] font-bold text-gray-900 shadow-sm hover:border-[#91D148]/40 transition-all flex justify-between items-center"
                    >
                      <span className={selectedTemplate ? "text-gray-900" : "text-gray-400 font-medium"}>
                        {selectedTemplate || "회의 템플릿을 선택해주세요"}
                      </span>
    ...
    <div>
      ...
      <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:bg-gray-50 transition-colors relative cursor-pointer">
        ...
        <div className="flex flex-col items-center justify-center gap-2">
          <span className="text-gray-400"><PaperclipIcon /></span>
          <p className="text-[14px] font-bold text-gray-600">클릭하거나 파일을 드래그하여 업로드하세요</p>
          <p className="text-[12px] text-gray-400 font-medium">최대 50MB (PDF, DOCX, PPTX 등)</p>
        </div>
      </div>
      {attachments.length > 0 && (
        <div className="mt-4 space-y-2">
          {attachments.map((file, i) => (
            <div key={i} className="flex items-center justify-between bg-white border border-gray-200 rounded-lg p-3 shadow-sm">
              <div className="flex items-center gap-2 overflow-hidden">
                <span className="text-gray-400 shrink-0"><PaperclipIcon /></span>
                <span className="text-[13px] font-bold text-gray-700 truncate">{file.name}</span>
                <span className="text-[11px] text-gray-400 shrink-0">({(file.size / 1024 / 1024).toFixed(2)} MB)</span>
              </div>
...
      <div className="px-8 py-7 space-y-6">
        {/* 💡 요청 사항에 맞춰 브리핑 카드 출력 항목 보강 */}
        <div className="border-b border-gray-100 pb-5">
          <h4 className="font-black text-[#111827] text-[24px] mb-4">{briefingCard.title}</h4>
          <div className="space-y-2 text-[15px] font-medium text-[#475467]">
            <p className="flex"><span className="w-20 font-black text-[#344054]">프로젝트</span> {briefingCard.project}</p>
            <p className="flex"><span className="w-20 font-black text-[#344054]">일시</span> {briefingCard.date} {briefingCard.time}</p>
            <p className="flex"><span className="w-20 font-black text-[#344054]">장소</span> {briefingCard.room}</p>
            <p className="flex"><span className="w-20 font-black text-[#344054]">참석자</span> {briefingCard.attendees.length > 0 ? briefingCard.attendees.join(", ") : "참석자 없음"}</p>
          </div>
        </div>

        <div className="text-[16px] font-bold text-[#475467] leading-relaxed">
...
{isTimeErrorModalOpen && createPortal(
  <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40">
    <div className="w-[420px] rounded-[28px] bg-white shadow-xl border border-gray-200 px-8 py-8 text-center">
      <div className="flex justify-center mb-4">
        <div className="w-14 h-14 rounded-full bg-red-50 flex items-center justify-center">
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#EF4444" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round">
            <circle cx="12" cy="12" r="10" />
            <line x1="12" y1="8" x2="12" y2="13" />
            <circle cx="12" cy="16.5" r="0.8" fill="#EF4444" stroke="none" />
          </svg>
        </div>
      </div>
      <h3 className="text-[22px] font-black text-gray-900 mb-3">시간 설정 오류</h3>
      <p className="text-[15px] text-gray-500 leading-relaxed mb-8">종료 시간은 시작 시간보다<br />이후로 설정되어야 합니다.</p>
      <button type="button" onClick={() => setIsTimeErrorModalOpen(false)} className="w-full py-3 rounded-xl bg-[#91D148] text-white font-black hover:opacity-90 transition-all">
        확인
      </button>
    </div>
  </div>,
...
  <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40" onClick={() => setIsTemplateModalOpen(false)}>
    <div className="w-[90%] max-w-[400px] rounded-[24px] bg-white shadow-xl border border-gray-200 p-6" onClick={(e) => e.stopPropagation()}>
      <h3 className="text-[18px] font-black text-gray-900 mb-4">템플릿 불러오기</h3>
...
                ? "bg-[#F4F9ED] text-[#91D148] font-black border border-[#91D148]/30"
                : "bg-gray-50 text-gray-700 font-bold border border-transparent hover:bg-gray-100"
            }`}
          >
            {template}
          </button>
        ))}
      </div>
      <button type="button" onClick={() => setIsTemplateModalOpen(false)} className="w-full mt-4 py-3 rounded-xl bg-gray-100 text-gray-500 font-black hover:bg-gray-200 transition-all">
        닫기
      </button>
    </div>
  </div>,
